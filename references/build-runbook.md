...
目标是与当前 Agent 路径无关的 VPS、且不修改本机网络时，允许 Agent 在影响确认后直接 SSH 执行。Agent 负责远端备份、Linux 命令、配置检查、新旧路径验证和失败回滚，不要求用户手动复制命令。
...
- 使用精确计划或目标可能承载 Agent 流量时，先按 [`control-channel-safety.md`](control-channel-safety.md) 确认依赖；共享控制通道未知时停止在计划阶段。独立远端 VPS 不因此被禁止直接 SSH。
