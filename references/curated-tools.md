...
Agent 不应一次把六个工具全部运行。当当前证据已指向一个能证伪主导假设的工具，且用户已授权它对应的外部数据披露或负载时，直接选择该工具，不再展示通用深度菜单。

只有当一个无法推断的用户选择会改变发送给谁的数据、测试目标或主动负载时，才按 `guided-dialogue.md` 给出 2–3 个互斥选项。可选项只包括与当前决策相关的路径，例如：

1. `继续内置扫描`：不调用外部工具，接受当前证据空白。
2. `使用已选定的增强工具`：说明它的目标、外部可见数据和负载。
3. `使用更高披露或负载的专项检测`：只在它能回答不同的必要问题时提供。

每次只推荐一个主要下一步。工具缺失时，只在“继续内置扫描”与“审核官方来源和版本后安装”会改变当前工作边界时请用户选择；不能因为工具缺失就自动执行安装脚本。
