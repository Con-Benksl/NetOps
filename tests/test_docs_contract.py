...
class DocumentationCommandTests(unittest.TestCase):
    ...
    def test_clean_release_gate_precedes_in_repository_artifact_builds(self):
        ...
        build = "scripts/reproducible_build.py ."
        for path in paths:
    ...
    def test_chinese_scan_output_explanation_precedes_change_workflow(self):
        ...
        scan = "python3 scripts/netopsctl.py scan client --output client.json"
        explanation = "上面的 `scan client` 命令默认只生成"
        safety = "python3 scripts/netopsctl.py safety assess"
        self.assertLess(text.index(scan), text.index(explanation))
        self.assertLess(text.index(explanation), text.index(safety))
