...
3. `审核后执行`: show impact and rollback details, then let the agent execute the authorized SSH transaction; use an exact plan only when the risk or transaction shape requires it.
...
4. State invariants: the agent's management path, existing nodes, panel access, SSH, default route, and unrelated exits remain unchanged.
...
6. Show the execution confirmation card from `<reference-root>/guided-dialogue.md` and ask once whether to execute, keep the plan only, or cancel. A shared-path guard returning `warn` still requires the risk card and explicit per-operation consent before disruptive work; it does not prohibit unrelated SSH maintenance.
7. After authorization, the agent runs the remote Linux commands itself. Do not ask the user to copy commands unless the action must occur on the user's local control plane or provider console.
