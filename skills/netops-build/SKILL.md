...
1. Apply `<reference-root>/control-channel-safety.md`. Classify the operation as a local control-plane change, a remote target carrying Codex traffic, or an unrelated remote VPS. Do not demand a full control-path proof for a clearly different VPS that does not change the Mac network.
